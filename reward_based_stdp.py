...
pygame.display.set_caption("R-STDP Optimized with Reward Curve")
...
if __name__ == '__main__':
    ...
    syn.connect(); syn.w = '0.3 + 0.4*rand()'
    ...
    for ep in range(episodes):
        ...
        episode_reward = 0  # To track total reward for this episode
        for _ in range(50):
            draw(state)
            ...
            if 0 <= nxt[0] < GRID_SIZE and 0 <= nxt[1] < GRID_SIZE and nxt not in obstacles:
                ...
                reward = -1
                collisions += 1
            episode_reward += reward  # Accumulate reward for this step
            syn.r = reward
            syn.w[:] = clip(syn.w + eta * syn.r * syn.elig, 0, 1)
            syn.elig *= 0.95
        ...
        reward_history.append(episode_reward)  # Store total reward for this episode
        print(f"[R-STDP] Ep {ep+1}: Acc={acc:.1f}%, Loss={loss:.2f}, Reward={episode_reward:.2f}")
    ...
    smoothed_reward = moving_average(reward_history, window_size)
    smoothed_episodes = range(window_size-1, episodes)

    # Plot accuracy, loss, and reward curves
    ...
    plt.plot(smoothed_episodes, smoothed_reward, label='Smoothed')
    plt.title('Reward'); plt.xlabel('Episode'); plt.ylabel('Total Reward'); plt.grid(True); plt.legend()
    plt.tight_layout(); plt.savefig('r_stdp_metrics_with_rewards.png')

    plt.figure(figsize=(8,2))
    for n, times in spike_mon.spike_trains().items():
        plt.vlines(times/ms, n+0.6, n+1.4)
    plt.xlabel('Time (ms)'); plt.ylabel('Neuron'); plt.title('Raster'); plt.savefig('r_stdp_rasters.png')